...
//! no NATS, frames no messages and chooses no transports. It asks the edge for
//! a population, walks it, hands parts of it back as clients would come and go,
//! and reads what comes down. Whether a command rides a datagram or a reliable
//! stream is a property of the command, decided by `EdgeClient`.
//!
//! ```text
...
use umwelt::net::{EdgeClient, EntityKind, FromEdge, RegionId};
use umwelt::{EntityId, Fixed, PacketReader, Pos3, RecordCodec, WorldConfig};
...
/// How long a receive waits before looking at whether it should stop.
...
fn play(
    ...
    let client = EdgeClient::new(conn, runtime.clone()).unwrap_or_else(|e| {
    ...
    for k in 0..observers + unattended {
        ...
        let at = home(k);
        match client.spawn(region, at, kind) {
    ...
    // What the edge has said about them. One receive loop, whichever transport
    // carried it.
    let spawned: std::sync::Mutex<Vec<(u32, RegionId, EntityId)>> =
        std::sync::Mutex::new(Vec::new());
    let removed: std::sync::Mutex<Vec<u32>> = std::sync::Mutex::new(Vec::new());
            ...
            while !stop.load(Ordering::Relaxed) {
                ...
                match message {
                    FromEdge::Spawned { handle, region, entity } => {
                        spawned.lock().expect("not poisoned").push((handle, region, entity));
                    }
                    ...
                    FromEdge::State { packet, .. } => {
                        ...
                        if let Some(reader) = PacketReader::new(&codec, &packet) {
                            records
                                .fetch_add(reader.updates().count() as u64, Ordering::Relaxed);
                        }
                    }
                    FromEdge::Message(_) => {}
                }
            ...
            if reported.elapsed() >= Duration::from_secs(1) {
                if churn > 0 && held.len() >= churn {
                    ...
                    for handle in &leaving {
                        if client.despawn(*handle).is_err() {
                            return;
                        }
                    }
                    ...
                    for _ in 0..leaving.len() {
                        ...
                        next_home += 1;
                        match client.spawn(region, at, EntityKind::Observer) {
                            Ok(handle) => {
